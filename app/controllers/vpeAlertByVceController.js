 ...
 */
portalControl.controller('vpeAlertByVceController', ['$scope', '$window', '$log', '$http', 'NgTableParams',
    function ($scope, $window, $log, $http, NgTableParams) {
        ...
        $scope.startTimeByVce = time[0];
        $scope.endTimeByVce = time[1];
        ...
        $scope.tableParamsByVce = createTableParams();
        $scope.modalTableParamsByVce = createTableParams();
        
        $scope.submitByVce = function() {
            $("#byVCE .submit").button('loading');
            $("#byVCE .report-result .table-area").hide();
            
            var startTimeStr = $scope.startTimeByVce.format('YYYYMMDDHHmmss');
            var endTimeStr = $scope.endTimeByVce.format('YYYYMMDDHHmmss');
            var url = '/gamma/api/quickWin/reportByVCE?startTime=' + startTimeStr
                      + "&endTime=" + endTimeStr
                      + "&vceName=" + encodeURIComponent($scope.vceName);
            console.log(url);
            $http({
                method: 'GET',
                url: url
                ...
                $scope.tableParamsByVce.settings({
                    dataset: data
                });
                
                prepareTopologyData(reportResult);
                
                $("#byVCE .report-result .table-area").show();
                $("#byVCE .submit").button('reset');
        ...
        var selectedVPE;
        $scope.openModalByVce = function(vpeName) {
            selectedVPE = vpeName;
            $(".modal-table").hide();
            $("#byVceModal").modal({
		backdrop: 'static',
            ...
            this.drawTopology = function(cust) {
                ...
                */
                ...
                $http({
                    ...
                    var custTopology = new CustTopology(container, 1, data, topologyData, $scope.modalTableParamsByVce);
                    custTopology.draw();
        ...
        var container = d3.select("#byVceTopology svg").attr("width", '100%').attr("height", 550);
        $("#byVceModal").on("shown.bs.modal", function() {
            container.selectAll("g").remove();
            container.attr("width", '100%');
            $(".modal-table").hide();
            var topology = new Topology(container, 1, selectedVPE, topologyData, 
            $scope.modalTableParamsByVce, custInfoTopo);
            topology.draw();
	});
        
        $scope.backButtonClickByVce = function() {
            container.selectAll("g").remove();
            container.attr("width", '100%');
            $(".modal-table").hide();
            var topology = new Topology(container, 1, selectedVPE, topologyData, 
