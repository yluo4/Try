 ...
 */
portalControl.controller('vpeAlertByCustomerController', ['$scope', '$window', '$log', '$http', 'NgTableParams',
    function ($scope, $window, $log, $http, NgTableParams) {
        ...
        $scope.startTimeByCustomer = time[0];
        $scope.endTimeByCustomer = time[1];
        ...
        $scope.tableParamsByCustomer = createTableParams();
        $scope.modalTableParamsByCustomer = createTableParams();
        
        $scope.submitByCustomer = function() {
            $("#byCustomer .submit").button('loading');
            $("#byCustomer .report-result .table-area").hide();
            
            var startTimeStr = $scope.startTimeByCustomer.format('YYYYMMDDHHmmss');
            var endTimeStr = $scope.endTimeByCustomer.format('YYYYMMDDHHmmss');
            var url = '/gamma/api/quickWin/reportByCustomer?startTime=' + startTimeStr
                      + "&endTime=" + endTimeStr
                      + "&custName=" + encodeURIComponent($scope.customerName);
            console.log(url);
            $http({
                method: 'GET',
                url: url
                ...
                $scope.tableParamsByCustomer.settings({
                    dataset: data
                });
                
                prepareTopologyData(reportResult);
                
                $("#byCustomer .report-result .table-area").show();
                $("#byCustomer .submit").button('reset');
        ...
        var selectedVPE;
        $scope.openModalByCustomer = function(vpeName) {
            selectedVPE = vpeName;
            $(".modal-table").hide();
            $("#byCustomerModal").modal({
		backdrop: 'static',
		keyboard: false
            });
        };
            ...
            this.drawTopology = function(cust) {
                ...
                */
                ...
                $http({
                    ...
                    var custTopology = new CustTopology(container, 1, data, topologyData, $scope.modalTableParamsByCustomer);
                    custTopology.draw();
        ...
        var container = d3.select("#byCustomerTopology svg").attr("width", '100%').attr("height", 550);
        $("#byCustomerModal").on("shown.bs.modal", function() {
            container.selectAll("g").remove();
            container.attr("width", '100%');
            $(".modal-table").hide();
            var topology = new Topology(container, 1, selectedVPE, topologyData, 
            $scope.modalTableParamsByCustomer, custInfoTopo);
            topology.draw();
	});
        
        $scope.backButtonClickByCustomer = function() {
            container.selectAll("g").remove();
            container.attr("width", '100%');
            $(".modal-table").hide();
            var topology = new Topology(container, 1, selectedVPE, topologyData, 
        ...
        /*
        var container = d3.select("#byCustomerTopology svg").attr("width", 868).attr("height", 380);
        $("#byCustomerModal").on("shown.bs.modal", function() {
            container.selectAll("g").remove();
            var topology = new Topology(container, 1, selectedVPE, alerts, $scope.modalTableParamsByCustomer);
            topology.draw();
	});
        */
