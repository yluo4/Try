/* 
 * This is for service path vce device
 * 
 */
...
TOPO.NodeVce2 = function(container_a, props_a) {
    ...
    this.draw = function(props_b) {
        ...
        // vce name
        if (props.vce != null) {
            var vceText = group.append("text")
                    .attr("x", vn.x)
                    .attr("y", vn.y)
                    .style("fill", vn.fill)
                    .style("font-size", vn.size)
                    .text(props.vce);
            var vceTextBox = vceText.node().getBBox();
            vn.x = ra1.x + (ra1.width - vceTextBox.width) / 2;
            vceText.attr("x", vn.x);
        }
...
TOPO.NodeVce2.prototype = Object.create(TOPO.Node.prototype);
TOPO.NodeVce2.prototype.constructor = TOPO.NodeVce2;